...
public class AnimatedTrimsMixinConfigPlugin implements IMixinConfigPlugin {
    @Override
            ...
            for(AnnotationNode node: annotationNodes) {
                if(node.desc.equals(Type.getDescriptor(ConditionalMixin.class))) {
                    ...
                    if(isModLoaded(modid)) {
                        AnimatedTrims.LOGGER.info("AnimatedTrimsMixinPlugin: " + className + " is" + (applyIfPresent ? " " : " not ") + "being applied because " + modid + " is loaded");
                        return applyIfPresent;
                    } else {
                        AnimatedTrims.LOGGER.info("AnimatedTrimsMixinPlugin: " + className + " is" + (!applyIfPresent ? " " : " not ") + "being applied because " + modid + " is not loaded");
                        return !applyIfPresent;
                    }
                }
            }
