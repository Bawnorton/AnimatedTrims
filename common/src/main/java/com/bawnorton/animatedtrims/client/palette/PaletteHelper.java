...
public abstract class PaletteHelper {
    ...
    public static List<Color> getPalette(Identifier identifier) {
        ...
        if (model == null) {
            AnimatedTrims.LOGGER.warn("Item " + item.getName().getString() + " has no model, using blank palette");
        ...
        if (sprite == null) {
            AnimatedTrims.LOGGER.warn("Model of item " + item.getName().getString() + " has no particle sprite, using blank palette");
            putPalette(identifier, BLANK_PALETTE);
            return BLANK_PALETTE;
        }
        ...
        if (content.getDistinctFrameCount().count() <= 0) {
            AnimatedTrims.LOGGER.warn("Sprite of item " + item.getName().getString() + " has no frames, using blank palette");
            putPalette(identifier, BLANK_PALETTE);
            return BLANK_PALETTE;
        }
    ...
    public static List<Color> existingResourceToPalette(Resource resource) {
        try {
            ...
            AnimatedTrims.LOGGER.error("Failed to read palette image", e);
            return BLANK_PALETTE;
        }
    }
    ...
    private abstract static class PaletteGenerator {
        ...
        public static List<Color> toPalette(NativeImage image) {
            if(image.getWidth() != 8 || image.getHeight() != 1) {
                AnimatedTrims.LOGGER.warn("Palette image is not 8x1 pixels, using blank palette");
                return BLANK_PALETTE;
            }
