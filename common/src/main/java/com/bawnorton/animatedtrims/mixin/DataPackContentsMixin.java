...
public abstract class DataPackContentsMixin {
    ...
    private static <T> Object addAnimatedTrimsToTrimTags(Object tagEntriesObj) {
        ...
        if (tagEntries.containsKey(ItemTags.TRIM_MATERIALS)) {
            ...
            entries.addAll(AnimatedTrimsItems.ITEMS.stream().map(item -> (RegistryEntry<T>) Registries.ITEM.getEntry(item)).toList());
