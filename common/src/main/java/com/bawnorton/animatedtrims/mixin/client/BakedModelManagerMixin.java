...
public abstract class BakedModelManagerMixin {
    ...
    private static Map<Identifier, Resource> addTrimModels(Map<Identifier, Resource> original) {
        ...
        for (Equipment equipment : equipmentSet) {
            ...
            if (armourType == null) {
                AnimatedTrims.LOGGER.warn("Item " + equipmentId + "'s slot type is not an armour slot type, skipping");
                continue;
            }
            if(Registries.ITEM.get(equipmentId) instanceof ElytraItem) {
                if(Compat.isElytraTrimsLoaded()) {
                    ...
                    AnimatedTrims.LOGGER.warn("Item " + equipmentId + " is an elytra, but elytratrims is not loaded, skipping");
                    continue;
                }
            }
            Resource resource = original.get(resourceId);
            if(resource == null) {
                AnimatedTrims.LOGGER.warn("Could not find resource " + resourceId + " for item " + equipmentId + ", skipping");
                continue;
            }
            try (BufferedReader reader = resource.getReader()) {
                JsonObject model = JsonHelper.fromJsonReader(reader, JsonObject.class);
                if (!model.has("textures")) {
                    AnimatedTrims.LOGGER.warn("Item " + equipmentId + "'s model does not have a textures parameter, skipping");
                    continue;
                }

                JsonObject textures = model.get("textures").getAsJsonObject();
                if (!textures.has("layer0")) {
                    AnimatedTrims.LOGGER.warn("Item " + equipmentId + "'s model does not have a layer0 texture, skipping");
                    continue;
                }
                ...
                while(true) {
                    ...
                    if(reachedEnd) {
                        overrideTextures.addProperty("layer" + layer, "minecraft:trims/items/" + armourType + "_trim_" + trimCount + "_" + AnimatedTrims.TRIM_ASSET_NAME);
                ...
                Identifier overrideResourceModelId = baseId.withPath("models/" + baseId.getPath() + "_" + AnimatedTrims.TRIM_ASSET_NAME + "_trim.json");
                ...
                DebugHelper.createDebugFile("models", equipmentId + "_" + AnimatedTrims.TRIM_ASSET_NAME + "_trim.json", JsonHelper.toJsonString(overrideResourceJson));
